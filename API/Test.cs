...
namespace UntitledCustomRoles.API
{
    public class CustomRole : ICustomRole
    {
        ...
        // INTERFACE
        int UId { get; set; } = 1;
        string Name { get; set; } = "Example";
        string CustomInfo { get; set; } = "Example CustomInfo";
        int SpawnChance { get; set; } = 20;
        RoleTypeId RoleType { get; set; } = RoleTypeId.ClassD;
        bool IgnoreRole { get; set; } = false;
        float Health { get; set; } = 120f;
        RoleTypeId RoleAppearance { get; set; } = RoleTypeId.None;
        Dictionary<EffectType, byte> Effects { get; set; } = new();
        bool CanEscape { get; set; } = true;
        RoleTypeId RoleAfterEscape { get; set; } = RoleTypeId.ChaosConcript;
        Vector3 Size { get; set; } = new Vector3(1, 1, 1);
        string BroadcastText { get; set; } = "You become an example!";
        ushort BroadcastDuration { get; set; } = 5;
        List<ItemType> Items { get; set; } = new()
        {
            ItemType.KeycardJanitor,
            ItemType.Coin
        };
        List<uint> CustomItems { get; set; } = new();
        Dictionary<AmmoType, ushort> Ammo { get; set; } = new();
        RoomType SpawnRoom { get; set; } = RoomType.Unknown;
        bool CanTrigger096 { get; set; } = true;
        bool CanTrigger173 { get; set; } = true;
        bool IgnoreGameSpawn { get; set; } = false;
        float DamageMiltiplier { get; set; } = 1;
